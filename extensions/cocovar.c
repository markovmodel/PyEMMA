...
int read_record_dat(reader r, double* record, int size)
{
  ...
  if(ferror(r.stream)) return -1;
  if(feof(r.stream)) return 0;
  ...
  if(isspace(ch)) state = SPACE; else state = NUMBER;
  ...
  while(ch!='\n' && ch!=-1) {
    switch(state) {
      case NUMBER:
        ...
        break;
      case SPACE:
        if(!isspace(ch)) { state = NUMBER; ungetc(ch, r.stream); }
        break;       
    }
    ch = getc(r.stream);
  }
...
{
  int i;
  if(fread(&i,4,1,stream)!=1) return -1;
  if(swap) 
    *value = reverse_int(i);
  else
    *value=i;
...
{
  float f;
  if(fread(&f,4,1,stream)!=1) return -1;
  if(swap) 
...
int init_dcd(reader *r, char *fname)
{
  ...
  if(fread(&key,4,1,r->stream)!=1) return -1;
  if(key!=84)  { r->swap=1; 
    printf("byte order is reversed, "); // debug
  } else r->swap=0;

  // Figure out how big the file is
  if(fseek(r->stream,0,SEEK_END)!=0) return -1;
  r->endoffile = ftell(r->stream);
  if(r->endoffile==-1) return -1;
  if(fseek(r->stream,4,SEEK_SET)!=0) return -1;

  // Read CORD
  if(fread(cord,1,4,r->stream)!=4) return -1;
  printf("sentinel %c%c%c%c, ",cord[0],cord[1],cord[2],cord[3]); // debug
  // Read NSET
  if(read_int(&nset,r->stream,r->swap)!=1) return -1;
  if(read_int(&istart,r->stream,r->swap)!=1) return -1;
  if(read_int(&nsavc,r->stream,r->swap)!=1) return -1;

  // Reposition to 40 from beginning; read namnf, number of free atoms
  printf("test namnf, "); // debug
  if(fseek(r->stream,40,SEEK_SET)!=0) return -1;
  if(read_int(&namnf,r->stream,r->swap)!=1) return -1;
  if(namnf!=0) {
  ...
  if(fseek(r->stream,84,SEEK_SET)!=0) return -1;
  if(read_int(&i,r->stream,r->swap)!=1) return -1;
  ...
  if(i==0)
    ...
    if(fseek(r->stream,48,SEEK_SET)!=0) return -1;
    if(read_int(&i,r->stream,r->swap)!=1) return -1;
    if(i == 1) r->charmm_extrablock = 1;
    printf("extrablock %d, ", r->charmm_extrablock); // debug
    // check for 4 dims
    if(fseek(r->stream,52,SEEK_SET)!=0) return -1;
    if(read_int(&i,r->stream,r->swap)!=1) return -1;
    if(i == 1) r->charmm_4dims = 1;
    printf("charmm_4dims %d, ", r->charmm_4dims); // debug
  }

  // Get the size of the next block, and skip it
  // This is the title
  if(fseek(r->stream,92,SEEK_SET)!=0) return -1;
  if(read_int(&newsize,r->stream,r->swap)!=1) return -1;
  //printf("newsize %d, ", newsize); // debug
  if(read_int(&numlines,r->stream,r->swap)!=1) return -1;
  //printf("numlines %d, ", numlines); // debug
  if(fseek(r->stream,numlines*80,SEEK_CUR)!=0) return -1;
  if(read_int(&newsize,r->stream,r->swap)!=1) return -1;
  //printf("newsize %d, ", newsize); // debug

  // Read in a 4, then the number of atoms, then another 4
  if(read_int(&i,r->stream,r->swap)!=1) return -1;
  if(read_int(&r->n,r->stream,r->swap)!=1) return -1;
  if(read_int(&i,r->stream,r->swap)!=1) return -1;
  printf("natoms %d ]\n", r->n); // debug
...
int read_record_dcd(reader r, double* record, int size)
{
  ...
  if(n>r.endoffile) { fprintf(stderr, "overshot file end\n"); return -1; }
  if(n==-1) return -1;
  if(n>=r.endoffile) return 0; 
  ...
  if(r.charmm && r.charmm_extrablock) {
    ...
    if(read_int(&blocksize,r.stream,r.swap)!=1) return -1;
    if(fseek(r.stream,blocksize,SEEK_CUR)!=0) return -1;
    if(read_int(&blocksize,r.stream,r.swap)!=1) return -1;
  }
  ...
  if(read_int(&blocksize,r.stream,r.swap)!=1) return -1;
  //printf("read X %d\n", blocksize); // debug
  if(3*blocksize/4 > size) return -1;
  for(i=0; i<blocksize/4; i++) 
    if(read_float(&record[3*i],r.stream,r.swap)!=1) return -1;
  if(read_int(&blocksize,r.stream,r.swap)!=1) return -1;
  //printf("read X %d\n", blocksize); // debug
  ...
  if(3*blocksize/4 > size) return -1;
  for(i=0; i<blocksize/4; i++) 
    if(read_float(&record[3*i+1],r.stream,r.swap)!=1) return -1;
  if(read_int(&blocksize,r.stream,r.swap)!=1) return -1;
  //printf("read Y %d\n", blocksize); // debug
  ...
  if(3*blocksize/4 > size) return -1;
  for(i=0; i<blocksize/4; i++) 
    if(read_float(&record[3*i+2],r.stream,r.swap)!=1) return -1;
  if(read_int(&blocksize,r.stream,r.swap)!=1) return -1;
  //printf("read Z %d\n", blocksize); // debug
...
PyArrayObject* unpack_array(PyObject *obj, int dim1, int dim2)
{
  ...
  if (dim1==1 && dim2==1) {
	cont_array = (PyArrayObject*)(PyArray_ContiguousFromAny(obj, PyArray_DOUBLE, 0, 0));   
	if (!cont_array) goto error;
  } else {
    if (dim2==1) {
      cont_array = (PyArrayObject*)(PyArray_ContiguousFromAny(obj, PyArray_DOUBLE, 1, 1)); 
      if (!cont_array) goto error;
      if (cont_array->dimensions[0]!=dim1) {
        PyErr_SetString(PyExc_ValueError, "Number of order parameters and length of mean/std/weights differ. Forgot time column?"); 
        goto error;
      }
    } else {
      cont_array = (PyArrayObject*)(PyArray_ContiguousFromAny(obj, PyArray_DOUBLE, 2, 2));
      if (!cont_array) goto error;
      if (cont_array->dimensions[0]!=dim1 || cont_array->dimensions[1]!=dim2 ) {
        PyErr_SetString(PyExc_ValueError, "Number of order parameters and length of mean/std/weights differ. Forgot time column?"); 
        goto error;
      }
    }
...
PyArrayObject* new_array(int dim1, int dim2) 
{
  ...
  if(!array) goto error;
  cont_array = (PyArrayObject*)PyArray_ContiguousFromAny(array, PyArray_DOUBLE, 0, 2);
  if(!cont_array) goto error;
  Py_DECREF(array); 
  /* initialize array content to zero */
  for(i=0; i<dim1*dim2; i++) D(cont_array,i) = 0.0; 
  return cont_array;
  
error:
  if(array) Py_DECREF(array);
  if(cont_array) Py_DECREF(cont_array);
  return NULL;
}
...
PyArrayObject* unpack_or_make(PyObject *dict, char *name, int dim1, int dim2)
{
  ...
  if (!array) { /* make default object */
    ...
    if(PyDict_SetItemString(dict, name, (PyObject*)cont_array)!=0) goto error;   
...
{
  int i;
  for(i=0; i<n; i++) D(mean,i) += record[i];
}

void update_var(const double *record, PyArrayObject* var, int n)
{
  int i;
  for(i=0; i<n; i++) D(var,i) += record[i]*record[i]; 
}
...
void update_tcov(const double *record, PyArrayObject* tcov, PyArrayObject* samples, int n, int m, double *shift, int lag)
{ 
  int i, j;
  ...
  if(m>=lag) {
    ...
    for(i=0; i<n; i++) { 
      for(j=0; j<n; j++) { 
        D(tcov,i*n+j) += record[j]*shift[((m-lag)%lag)*n+i];
      }
    }     
  }
  for(i=0; i<n; i++) shift[(m%lag)*n+i] = record[i];
}
...
static PyObject *run(PyObject *self, PyObject *args) {
  ...
  if(!PyArg_ParseTuple(args, "sO!iiiii", &fname, &PyDict_Type, &stats, &do_mean, &do_var, &do_cov, &do_tcov, &lag)) goto error;  
  
  /* open date file and get record size */
  input = reader_by_fname(fname);
  n = input.init(&input, fname);
  if(n<1) { PyErr_SetFromErrno(PyExc_IOError); goto error; }

  record = (double*)malloc(n*sizeof(double));
  if(!record) { PyErr_NoMemory(); goto error; }
  ...
  if(do_tcov) {
    ...
    if(!tcov_samples) goto error;
    shift = (double*)malloc(n*lag*sizeof(double));
    if(!shift) { PyErr_NoMemory(); goto error; }
    for(i=0; i<n*lag; i++) shift[i] = 0;
  }
  ...
  j = 0;
  while(1) {
    if(ferror(input.stream)) { PyErr_SetFromErrno(PyExc_IOError); goto error; }
    c=input.read_record(input,record,n);
    if(c==0) break; /* empty line or eof */
    if(c!=n) { PyErr_SetString(PyExc_ValueError, "Unexpected line format or read error."); goto error; }
    ...
    if(!do_mean)
      for(i=0;i<n;i++) record[i]-=D(mean,i);
  ...
  if(input.stream) fclose(input.stream);
  if(record) free(record);
  if(shift) free(shift);
  if(samples) Py_DECREF(samples);
  if(mean) Py_DECREF(mean); 
  if(var) Py_DECREF(var); 
  if(cov) Py_DECREF(cov); 
  if(tcov) Py_DECREF(tcov);
  if(tcov_samples) Py_DECREF(tcov_samples);
...
static PyObject *stream(PyObject *self, PyObject *args) {
  ...
  if(!PyArg_ParseTuple(args, "ssOOii", &fin, &fout, &py_mean, &py_W, &max, &time_column)) goto error;  

  time_column = !!time_column; /* convert to boolean */
  if(max<1) { PyErr_SetString(PyExc_ValueError, "max < 1"); goto error; }
  ...
  if(max>m) { PyErr_SetString(PyExc_ValueError, "requested output record length (max) > input record length"); goto error; }

  /* convert numeric parameters to arrays */
  mean = unpack_array(py_mean, m, 1);
  W    = unpack_array(py_W, m, m);
  if(!mean || !W) goto error; 
  ...
  /* open output file */
  out = fopen(fout, "w");
  if(!out)  { PyErr_SetFromErrno(PyExc_IOError); goto error; }
  ...
  while(1) {
    ...
    for(i=0; i<m; i++) record[i] = record[i+time_column]-D(mean,i);
    for(j=0; j<max; j++) {
      sum = 0;
      for(i=0; i<m; i++) sum+=record[i]*D(W,m*i+j); 
      fprintf(out, "%f", sum);
      if(j!=max-1) fprintf(out, " ");
    }
    fprintf(out, "\n");
  }
...
#define STREAM_USAGE \
  ...
  "stream(fin, fout, mean, weights, max_proj, time_column)\n\n" \
...
{
     {"run", run, METH_VARARGS, RUN_USAGE},
     {"stream", stream, METH_VARARGS, STREAM_USAGE},     
     {NULL, NULL, 0, NULL}
};
