...
static PyObject *cluster(PyObject *self, PyObject *args) {
    int debug;
    debug = 0;
    if(debug == 1) printf("KMEANS: \n----------- cluster called ----------\n");
    if(debug == 1) printf("KMEANS: declaring variables...");
    ...
    if(debug == 1) printf("KMEANS: initializing some of them...");
    ...
    if(debug == 1) printf("KMEANS: attempting to parse args...");
    ...
    /* import chunk */
    if(debug == 1) printf("KMEANS: importing chunk...");
    ...
    if(debug == 1) printf("done with N_frames=%d, dim=%d\n", N_frames, dim);

    if(debug == 1) printf("KMEANS: creating metric function...");
    ...
    if(debug == 1) printf("KMEANS: importing list of cluster centers...");
    ...
    for(i = 0; i < N_centers; ++i) {
        ...
        if(debug == 1) printf("%d", l++); // 8
        centers[i] = (float*)PyArray_DATA(np_item);
        if(debug == 1) printf("%d", l++); // 9
    }
    if(debug == 1) printf("done, k=%d\n", N_centers);

    /* initialize centers_counter and new_centers with zeros */
    if(debug == 1) printf("KMEANS: initializing calloc centers counter and new_centers stuff...");
    ...
    /* do the clustering */
    if(debug == 1) printf("KMEANS: performing the clustering...");
    ...
    if(debug == 1) printf("KMEANS: creating return_new_centers...");
    ...
    if(debug == 1) printf("KMEANS: attempting memcopy...");
    memcpy(arr_data, new_centers, PyArray_ITEMSIZE((PyArrayObject*) return_new_centers) * N_centers * dim);
    if(debug == 1) printf("done\n");
    if(debug == 1) printf("KMEANS: increasting ref to return_new_centers thingy...");
