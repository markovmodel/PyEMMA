...
class TestMfptDense(unittest.TestCase):
    ...
    def test_mfpt(self):
        x=mfpt(self.P, 0)
        np.testing.assert_allclose(x, self.m0)

        x=mfpt(self.P, 1)
        np.testing.assert_allclose(x, self.m1)

        x=mfpt(self.P, 2)
        np.testing.assert_allclose(x, self.m2)

        x=mfpt(self.P, [0,1])
        np.testing.assert_allclose(x, self.m01)

        x=mfpt(self.P, [1,2])
        np.testing.assert_allclose(x, self.m12)

    def test_mfpt_between_sets(self):
        x=mfpt(self.P,[1,2],origin=0)
        np.testing.assert_allclose(x,self.o0t12)

        x=mfpt(self.P,[0,1],origin=2)
        np.testing.assert_allclose(x,self.o2t01)

        x=mfpt(self.P,2,origin=[0,1])
        np.testing.assert_allclose(x,self.o01t2)

        x=mfpt(self.P,0,origin=[1,2])
        np.testing.assert_allclose(x,self.o12t0)

################################################################################
# Sparse
################################################################################

class TestMfptSparse(unittest.TestCase):
    ...
    def setUp(self):
        ...
        q3=0.6
        """3x3 birth and death transition matrix"""
        P=np.array([[1.0-p1, p1, 0.0],\
                        [q2, 1.0-q2-p2, p2],\
                        [0.0, q3, 1.0-q3]]) 
        self.P=scipy.sparse.coo_matrix(P)
        """Vector of mean first passage times to target state t=0"""
        self.m0=np.array([0.0, (p2+q3)/(q2*q3), (p2+q2+q3)/(q2*q3)])
        """Vector of mean first passage times to target state t=1"""
        self.m1=np.array([1.0/p1, 0.0, 1.0/q3])
